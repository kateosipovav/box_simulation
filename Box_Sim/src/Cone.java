...
public class Cone {
	...
	public double getSurfaceArea() {
		System.out.println("The surface area this time is "
				+ (Math.PI * radius * (radius + Math.sqrt(Math.pow(radius, 2) + Math.pow(height, 2)))));
	...
	private double getVolumeAroundCone() {
		...
		if ((getHeight() + Plant.HEIGHT < Cubicle.SIDE) && (getRadius() + Plant.HEIGHT < Cubicle.SIDE / 2)) {
			...
			System.out.println(
					"The volume around the cone this time is " + ((h * Math.PI * Math.pow(r, 2)) - getVolume()));
			return ((h * Math.PI * Math.pow(r, 2)) - getVolume());
		}
		// Situation 2
		else if ((getHeight() + Plant.HEIGHT < Cubicle.SIDE) && !(getRadius() + Plant.HEIGHT < Cubicle.SIDE / 2)) {
			// when cut we can think of the resulting figure as a cylinder and a
			// cone
			// find the bottom side of the piece that sticks out, and determine
			// the height of that piece
